...
## Adapted from OGB code: https://github.com/snap-stanford/ogb/blob/master/examples/linkproppred/citation2/sampler.py
    ...
    def inference(self, x_all, subgraph_loader, device):
        ...
        for i in range(self.num_layers):
            ...
            for batch_size, n_id, adj in subgraph_loader:
                torch.cuda.empty_cache()
                gc.collect()
                edge_index, sth, size = adj
                print("Allocation is : torch.cuda.memory_allocated: %fGB" % (torch.cuda.memory_allocated(0) / 1024 / 1024 / 1024))
                x = x_all[n_id]
                x_target = x[:size[1]]
                x = self.convs[i]((x.to(device), x_target.to(device)), edge_index.to(device))
                if i != self.num_layers - 1:
                    x = F.relu(x)
...
def gcn_train(model, predictor, loader, optimizer, device):
    model.train()
...
@torch.no_grad()
def gcn_test(model, predictor, data, split_edge, evaluator, batch_size, device, seed, MODEL, DATASET):
    predictor.eval()
    ...
    save_embedding_data(h, seed, MODEL, DATASET)
...
def save_embedding_data(node_embbeding, seed, MODEL, DATASET):
    file_name = f'model_results/{MODEL}_{DATASET}/{str(seed)}_emb.pt'
    torch.save(node_embbeding.detach(), file_name)
    return
...
if __name__ == "__main__":
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            if MODEL == 'GCN':
                print('In GCN train')
                loss = gcn_train(model, predictor, loader, optimizer, device)
            else:
            ...
            if epoch % args.eval_steps == 0:
                if MODEL == 'GCN':
                    result = gcn_test(model, predictor, data, split_edge, evaluator,
                              batch_size=64 * 1024, device=device, seed=seed, MODEL=MODEL, DATASET=DATASET)
                else:
                ...
                training_log = f'{file_path}/training_log.txt'
                with open(training_log, 'a') as file:
                    file.write(f'Run: {run + 1:02d}, '
                      f'Epoch: {epoch:02d}, '
                      f'Loss: {loss:.4f}, '
        ...
        torch.cuda.empty_cache()
        model_path = f'model_results/{MODEL}_{DATASET}/{MODEL}_{seed}_model.pt'
        torch.save(model.state_dict(), model_path)

        predictor_path = f'model_results/{MODEL}_{DATASET}/{MODEL}_predictor_{seed}.pt'
